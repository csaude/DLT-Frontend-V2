...
import { View, KeyboardAvoidingView, ScrollView,
    Text} 
    from 'react-native';
import { Box, Heading, Divider, Avatar, 
    Icon, Flex } 
    from "native-base";
                ...
                <View style={styles.user}>
                    <View style={styles.containerForm}>
                        ...
                        <Flex direction="column" mb="2.5" mt="1.5" _text={{color: "coolGray.800"}}>
                            ...
                            <Text> <Text style={styles.txtLabel}>Perfil: </Text> {profile?.name}</Text>
                            ...
                            <Text> <Text style={styles.txtLabel}>Província(s): </Text> {provinces?.map(p => p.name+', ')} </Text>
                            
                            <Text> <Text style={styles.txtLabel}>Distrito(s): </Text> {districts?.map(d => d.name+', ')} </Text>
                            <Text> <Text style={styles.txtLabel}>Posto(s) Administrativo(s): </Text> {localities?.map(d => d.name+', ')} </Text>
                            <Text> <Text style={styles.txtLabel}>Alocação: </Text> { uss?.name } </Text>
...
const enhance = withObservables([], () => ({
    ...
    profiles: database.collections
      .get("profiles")
      .query(),
    provinces: database.collections
      .get("provinces")
      .query(),
    districts: database.collections
      .get("districts")
      .query(),
