...
const ChangePassword: React.FC = ({ route }: any) => {
  ...
  const errorMessage = params.passwordExpired
    ? "Alteração da senha é obrigatório a cada 6 meses "
    : "Alteração da senha é obrigatório no primeiro login ";
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      const userDetailsQ = await userDetails
        ...
        .fetch();
      const usersQ = await users
        .query(Q.where("online_id", parseInt(userDetail.user_id)))
        .fetch();
