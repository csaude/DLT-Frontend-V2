...
const OrganizationList: React.FC = () => {
  ...
  const organizationSort = (data: any) => {
    ...
    const sortedOrganizations = orgs.sort(
      (ser1, ser2) => ser2.code - ser1.code
    );
  ...
  const handleAdd = () => {
    form
      .validateFields()
      .then(async (values) => {
        const organization: any = selectedOrganization
        ...
        if (selectedOrganization === undefined) {
          ...
          const result = organizations.some(
            (organizations) =>
              organizations.name.replace(/\s+/g, "").toLowerCase() ===
              values.name.replace(/\s+/g, "").toLowerCase()
          );
          if (result) {
            message.error({
              content: " Organização repetida.",
      ...
      .catch(() => {
        message.error({
          content: "Não foi possível Registar o Organização",
          className: "custom-class",
          style: {
            marginTop: "10vh",
          },
