...
const ServicesList: React.FC = () => {
    ...
    form
      ...
      .then(async (values) => {
        ...
        if (selectedService === undefined) {
          ...
          const result = services.some(
            (services) =>
              services.name.replace(/\s+/g, "").toLowerCase() ===
              values.name.replace(/\s+/g, "").toLowerCase()
          );
          if (result) {
            message.error({
              content: " Serviço repetido.",
      ...
      .catch(() => {
        message.error({
          content: "Não foi possível Registar o Serviço",
          className: "custom-class",
          style: {
            marginTop: "10vh",
          },
  ...
  return (
    <>
      <Title />
      <Card
        title="Lista de Serviços DREAMS"
        bordered={false}
      ...
      >
        <ConfigProvider locale={ptPT}>
          <Table rowKey="id" columns={columns} dataSource={services} bordered />
        </ConfigProvider>
      </Card>
      <ServiceForm
        form={form}
