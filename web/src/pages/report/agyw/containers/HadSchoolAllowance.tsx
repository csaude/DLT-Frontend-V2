...
const HadSchoolAllowance = ({ districtId }) => {
  ...
  const total = responseData[districtId]["had-school-allowance"].total;
  const hadSchoolAllowance =
    responseData[districtId]["had-school-allowance"].totals;

  const arrTotals = Object.keys(hadSchoolAllowance).map((key) => ({
    key,
    value: hadSchoolAllowance[key],
  }));
