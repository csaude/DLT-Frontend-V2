...
const LocalityList: React.FC = () => {
    ...
    const [localitiesModalVisible, setLocalitiesModalVisible] = useState<boolean>(false);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const sortedLocalities = localities.sort((ser1, ser2) => ser2.id - ser1.id);
    ...
    const handleAdd = () => {
        form.validateFields().then(async (values) => {
            ...
            if (selectedLocality === undefined) {
                ...
                const result = locality.some(locality => locality.name === values.name.trim());
                if (result){
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record) => (
                <Space>
                    <Button type="primary" icon={<EditOutlined />} onClick={() => onEditLocality(record)} />
                </Space>
            )
        }
    ]
    ...
    return (
        <>
            <Title />
            <Card title="Postos Administrativos" bordered={false} headStyle={{ color: "#17a2b8" }}
                extra={
                    <Space>
                        <Button type="primary" icon={<PlusOutlined />} onClick={() => handleLocalityModalVisible(true)}>
            ...
            <LocalityForm form={form} locality={selectedLocality} modalVisible={localitiesModalVisible} handleModalVisible={handleLocalityModalVisible} handleAdd={handleAdd} />
        </>
    );
}
