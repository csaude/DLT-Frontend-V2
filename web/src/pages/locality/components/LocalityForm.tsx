...
const LocalityForm = ({ form, locality, modalVisible, handleModalVisible, handleAdd }) => {
                ...
                <Row gutter={12}>
                    <Col span={8}>
                        <Form.Item
                            ...
                            initialValue={locality === undefined ? "" : locality?.district?.province?.id.toString()}
                        >
                    ...
                    <Col span={8}>
                        <Form.Item
                            ...
                            label="Distrito"
                            rules={[{ required: true, message: RequiredFieldMessage }]}
                            initialValue={locality === undefined ? "" : locality?.district?.id.toString()}
                        >
                            <Select 
                                disabled={!enabledDistricts}
                            >
                            {districts?.map(item => (
                                <Option key={item.id}>{item.name}</Option>
                            ))}
                ...
                <Row gutter={8}>
                    <Col className="gutter-row" span={8}>
                        <Form.Item
                            ...
                            initialValue={locality === undefined ? "1" : locality?.status?.toString()}
                        >
