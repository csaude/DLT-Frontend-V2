...
export async function geNewlyEnrolledAgywAndServicesSummaryReportGenerated(
  ...
  const url = `/api/agyw-prev/getNewlyEnrolledAgywAndServicesSummary?province=${province}&districts=${districts}&startDate=${startDate}&endDate=${endDate}&username=${username}`;
  const res = await select(url);
  return res;
}
